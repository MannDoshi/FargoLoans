...
public class EmployeCardController {
	...
	public Map<String, Boolean> deleteProduct(@PathVariable(value = "id") Long pId) throws ResourceNotFoundException {
		EmployeeCard e = ecservice.getSingleEmployeeCard(pId)
				.orElseThrow(() -> new ResourceNotFoundException("EmployeeCard not found for this Id :" + pId));
	...
	@PutMapping("/{id}")
	public ResponseEntity<EmployeeCard> updateEmployeeCard(@PathVariable(value = "id") Long pId,
			@Validated @RequestBody EmployeeCard ec) throws ResourceNotFoundException {
		EmployeeCard employeeCard = ecservice.getSingleEmployeeCard(pId)
